...
const calculator_service_js_1 = require("../services/calculator.service.js");
...
router.post('/work-hours', async (req, res) => {
    try {
        ...
        const workHours = await calculator_service_js_1.calculatorService.calculateWorkingHours(start, end);
        res.json({
            workHours,
            period: { start, end }
        });
...
router.get('/holidays', async (req, res) => {
    try {
        ...
        const holidays = await calculator_service_js_1.calculatorService.getHolidaysList(start, end);
        res.json({ holidays });
    }
    catch (error) {
